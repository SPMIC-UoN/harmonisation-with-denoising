...
Hierarchical Bayesian power simulation for H1/2a.

Model:
    y_{s,r,c} ~ Normal(alpha0 + alpha1*den + u_s , sigma)

Usage (example):
    python H1a.py --output results_h1a.csv --replicates 50 --draws 1000 --tune 500 --cores 2
"""
...
# Simulation function
# ---------------------------
def simulate_data(S=10, R_per_subject=6800, alpha0=0.0, alpha1=0.20,
                  tau_u=0.6, sigma=1.0, seed=None):
    ...
    for s in range(S):
        u_s = rng.normal(0.0, tau_u)
        den_s = den_template.copy()
        # No random slope in data generation
        y_s = alpha0 + alpha1 * den_s + u_s + rng.normal(0.0, sigma, size=R_per_subject)
        all_y.append(y_s)
        all_subj.append(np.full(R_per_subject, s, dtype=int))
        all_den.append(den_s)

    y = np.concatenate(all_y)
    subj = np.concatenate(all_subj)
    den = np.concatenate(all_den)

    return dict(y=y, subj=subj, den=den)
...
def fit_and_compute_bf(data, prior_sd_alpha1=1.0, draws=2000, tune=2000, chains=4, cores=1, random_seed=None):
    ...
    with pm.Model() as model:
        ...
        # Random intercept only (non-centered)
        # Note: keep the same CLI arg `tau_b` for compatibility but it is unused here.
...
# ---------------------------
def _replicate_worker(i, S, R_per_subject, alpha1, tau_u, tau_b, sigma, prior_sd_alpha1, draws, tune, chains, cores, rng):
    seed = int(rng.integers(1, 2**30))
    data = simulate_data(S=S, R_per_subject=R_per_subject, alpha1=alpha1, tau_u=tau_u, sigma=sigma, seed=seed)
...
# ---------------------------
def estimate_power_hierarchical(S=9, R_per_subject=6800, alpha1=0.20,
                                tau_u=0.6, sigma=1.0,
                                prior_sd_alpha1=1.0,
                                replicates=200,
    ...
    if parallel and n_workers > 1:
        with mp.Pool(processes=n_workers) as pool:
            func = partial(_replicate_worker, S=S, R_per_subject=R_per_subject, alpha1=alpha1,
                           tau_u=tau_u, tau_b=0.0, sigma=sigma, prior_sd_alpha1=prior_sd_alpha1,
        ...
        for i in tqdm(range(replicates), desc="MC replicates", position=progress_pos):
            res = _replicate_worker(i, S, R_per_subject, alpha1, tau_u, 0.0, sigma, prior_sd_alpha1, draws, tune, chains, cores, rng)
            results.append(res)
...
# ---------------------------
def parse_args():
    p = argparse.ArgumentParser(description="H1a Bayesian power simulation (hierarchical model)")
    ...
    p.add_argument('--R_per_subject', type=int, default=6800)
...
def main():
    ...
    logging.info(f'Starting H1a power simulation: replicates={args.replicates}, S={args.S}, R_per_subject={args.R_per_subject}')
